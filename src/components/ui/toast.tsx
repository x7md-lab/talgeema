...
function ToastList() {
	...
	return (
		<Toast.Portal data-slot="toast-portal">
			<Toast.Viewport
				className="fixed top-4 right-4 bottom-auto mx-auto flex w-[calc(100%_-_2rem)] sm:right-8 sm:top-8 sm:w-[356px] z-[9999]"
			...
			>
				{toasts.map((toast) => (
					<Toast.Root
						...
						data-slot="toast"
						className={cn(
							"bg-popover absolute right-0 top-0 left-auto z-[calc(1000-var(--toast-index))] mr-0 flex w-full items-center justify-between gap-1.5 rounded-lg border bg-clip-padding p-4 shadow-lg transition-all [transition-property:opacity,transform] duration-200 ease-out select-none",
							"after:absolute after:top-full after:left-0 after:h-[calc(var(--gap)+1px)] after:w-full after:content-['']", // Fill the gap between toasts
							...
							"data-[ending-style]:data-[swipe-direction=up]:[transform:translateY(calc(var(--toast-swipe-movement-y)-150%))] data-[expanded]:data-[ending-style]:data-[swipe-direction=up]:[transform:translateY(calc(var(--toast-swipe-movement-y)-150%))]", // Up swipe animation
							toast.type === "success" &&
